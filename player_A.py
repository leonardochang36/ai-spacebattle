...
class Player:
    def __init__(self, ship_pos, goal_side):
        ...
        self.my_ship_pos = ship_pos


    def next_move(self, current_state):
        """ Function that computes the next move of your ship

        Implement your algorithm here. This will be the only function
        used by the GameCore. Be aware of abiding all the game rules.

        Returns:
            dict: coordinates of next position of your ship.
        """

        # update my ship pos
        # I need to do this because GameCore moves my ship randomly
        self.my_ship_pos = current_state['ship1_pos'] if self.my_goal == 'left' \
                                                              else current_state['ship2_pos']
        ...
        if pt_in_roi:
            ...
            target_pos = utils.aim(pt_in_roi[0], pt_in_roi[1],
                                   ...
                                   current_state['ship_radius'])

            # move to target position, taking into account the max. ship speed
            if target_pos != self.my_ship_pos:
                direction_vector = {'x': target_pos['x'] - self.my_ship_pos['x'],
                                    'y': target_pos['y'] - self.my_ship_pos['y']}
                direction_vector = {k: v / utils.vector_l2norm(direction_vector)
                                    for k, v in direction_vector.items()}

                movement_dist = min(current_state['ship_max_speed'] * current_state['delta_t'],
                                    utils.distance_between_points(target_pos, self.my_ship_pos))
                direction_vector = {k: v * movement_dist
                                    for k, v in direction_vector.items()}
                new_ship_pos = {'x': self.my_ship_pos['x'] + direction_vector['x'],
                                  'y': self.my_ship_pos['y'] + direction_vector['y']}

                # check if computed new position in not inside goal area
                # check if computed new position in inside board limits
                if utils.is_inside_goal_area_ship(new_ship_pos, current_state) is False and \
                     utils.is_out_of_boundaries_ship(new_ship_pos, current_state) is None:
                    self.my_ship_pos = new_ship_pos
